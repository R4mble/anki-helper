...
function Stop-HotkeyProcesses {
    Get-CimInstance Win32_Process -Filter "Name='powershell.exe' OR Name='cmd.exe'" |
        Where-Object { $_.CommandLine -like '*win-cut-bbc-pic-hotkey.ps1*' -or $_.CommandLine -like '*run-bbc-pic-hotkey.bat*' } |
        ForEach-Object {
...
function Get-HotkeyProcessCount {
    return @(Get-CimInstance Win32_Process -Filter "Name='powershell.exe' OR Name='cmd.exe'" |
            Where-Object {
                $_.CommandLine -like '*win-cut-bbc-pic-hotkey.ps1*' -or $_.CommandLine -like '*run-bbc-pic-hotkey.bat*'
            }).Count
}
...
function Start-HotkeyDirect {
    $psPath = Join-Path $PSScriptRoot 'win-cut-bbc-pic-hotkey.ps1'
