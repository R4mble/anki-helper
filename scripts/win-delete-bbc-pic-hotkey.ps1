...
$mutex = New-Object System.Threading.Mutex($false, 'Global\AnkiHelper_BbcPicDeleteHotkey')
...
public class AnkiHelperDeleteHotkeyWindow : NativeWindow, IDisposable {
    ...
    public AnkiHelperDeleteHotkeyWindow(int hotkeyId, uint mods, uint vk, string label, string callbackUrl) {
        _hotkeyId = hotkeyId;
        _mods = mods;
        _vk = vk;
        Label = label;
...
$candidates = @(
    @{ Label = 'Ctrl+Alt+Shift+F5'; Mods = [uint32]7; Vk = [uint32]0x74 },
    @{ Label = 'Ctrl+Alt+Shift+F4'; Mods = [uint32]7; Vk = [uint32]0x73 },
    @{ Label = 'Ctrl+Shift+F8'; Mods = [uint32]6; Vk = [uint32]0x77 },
    @{ Label = 'Ctrl+Shift+F7'; Mods = [uint32]6; Vk = [uint32]0x76 }
)

$callbackUrl = "http://127.0.0.1:$Port/api/anki/delete-bbcnews-pic"
$window = $null
$registered = $false
...
try {
    ...
    foreach ($item in $candidates) {
        ...
        try {
            $window = New-Object AnkiHelperDeleteHotkeyWindow($hotkeyId, $item.Mods, $item.Vk, $item.Label, $callbackUrl)
